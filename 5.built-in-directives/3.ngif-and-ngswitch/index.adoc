...
[IMPORTANT]
====
The difference between `[hidden]='true'` and `*ngIf='false'` is that the first method simply _hides_ the element. The second method with `ngIf` _removes_ the element completely from the DOM.
====
